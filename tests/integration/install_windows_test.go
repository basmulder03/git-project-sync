...
func TestWindowsInstallScriptsContainTaskRegistrationFlow(t *testing.T) {
	...
	for _, want := range []string{"Invoke-WebRequest", "releases/latest/download", "synctui_windows_amd64.exe", "raw.githubusercontent.com", "scripts/install.ps1", "synctui:", "Next steps:", "docs/getting-started/first-run-onboarding.md", "$syncctlPath sync all --dry-run"} {
		if !strings.Contains(string(bootstrapContent), want) {
			t.Fatalf("bootstrap.ps1 missing %q", want)
		}
	}

	for _, want := range []string{"schtasks", "/Create", "/Query", "run-syncd.cmd", "Add-ToPath", "SetEnvironmentVariable", "Set-Content"} {
