...
func TestLinuxInstallScriptsContainServiceRegistrationFlow(t *testing.T) {
	...
	for _, want := range []string{"releases/latest/download", "scripts/install.sh", "syncd_linux_${ARCH}", "synctui_linux_${ARCH}", "synctui:", "Next steps:", "docs/getting-started/first-run-onboarding.md", "syncctl sync all --dry-run"} {
